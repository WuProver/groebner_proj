...
def addIndentationLinters (linter : IndentationLinters) : IO Unit := do
  indentationLinterRef.modify (·.add linter)

def runLinterOn : IndentationLinter :=
  fun info limit ↦ do
    let linters ← indentationLinterRef.get
