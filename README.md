...
- [`MonomialOrder.remainder_eq_zero_iff_mem_ideal_of_isGroebner`](https://wuprover.github.io/groebner_proj/docs/find/#doc/MonomialOrder.remainder_eq_zero_iff_mem_ideal_of_isGroebner): given a Gröbner basis $G$ of an ideal $I\subseteq k[x_i: i\in \sigma]$, a polynomial $p\in k[x_i: i\in \sigma]$, and a remainder $r$ of $p$ on division by $G$, then $r = 0$ if and only if $p\in I$.
...
- [`MonomialOrder.ideal_eq_span_of_isGroebner`](https://wuprover.github.io/groebner_proj/docs/find/#doc/MonomialOrder.ideal_eq_span_of_isGroebner): if $G$ is a Gröbner basis of $I\subseteq k[x_i:i\in\sigma]$, then $I=\langle G\rangle$.
